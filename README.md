...
The production deployment uses Nginx, HTTPS, systemd and a prebuilt local index. PDF upload is enabled with a 50 MB limit, per-IP rate limiting, asynchronous status polling, serialized indexing, and automatic PyPDF2 fallback when MinerU is unavailable. Public search and answer requests support GET to improve reliability across restrictive network paths.
